...
public class DeviceProfile {
    ...
    /** Returns the AllApps padding in the specified orientation */
    Rect getAllAppsPadding() {
        return getWorkspacePadding(isLandscape ? CellLayout.LANDSCAPE : CellLayout.PORTRAIT);
    }

    Rect getAllAppsPadding(int orientation) {
        Rect searchBarBounds = getSearchBarBounds(orientation);
        Rect padding = new Rect();
        if (orientation == CellLayout.LANDSCAPE &&
                transposeLayoutWithOrientation) {
...
//                int paddingBottom = hotseatBarHeightPx + pageIndicatorHeightPx;
    ...
    public void layout(Launcher launcher) {
        ...
        padding = getAllAppsPadding(orientation);
        lp = (FrameLayout.LayoutParams) host.getLayoutParams();
        lp.gravity = Gravity.TOP;
//        lp.bottomMargin = padding.bottom*2;
        host.setLayoutParams(lp);
        host.setPadding(padding.left, padding.top, padding.right, padding.bottom);
        if (host != null) {
